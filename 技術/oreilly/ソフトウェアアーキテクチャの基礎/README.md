...
- [第一部 基礎](#第一部-基礎)
  ...
  - [5. アーキテクチャ特性を明らかにする](#5-アーキテクチャ特性を明らかにする)
    - [5.1 ドメインの関心事からアーキテクチャ特性を捉える](#51-ドメインの関心事からアーキテクチャ特性を捉える)
    - [5.3 事例：シリコンサンドイッチ](#53-事例シリコンサンドイッチ)
      - [5.3.1 明示的な特性](#531-明示的な特性)
      - [5.3.2 暗黙的な特性](#532-暗黙的な特性)
...
サポートするアーキテクチャ特性の数は可能な限り絞る  
**全てのアーキテクチャ特性をサポートするのはアンチパターン**  
より良い方法は、主なステークホルダーに重要な上位３つの特性を選んでもらうこと
ステークホルダーとアーキテクトでは同じゴールでも異なる表現をする

**表5-1 ドメインの関心事からアーキテクチャ特性の翻訳**
| ドメインの関心事   | アーキテクチャ特性                                                                       |
| ------------------ | ---------------------------------------------------------------------------------------- |
...
- 記述
  - 全国展開しているサンドイッチ点が、(電話注文に加えて)オンライン注文を可能にしたい 
- ユーザ数
  - 数千。いずれは数百万
- 要件
  - 要件1：ユーザが注文すると、サンドイッチを受け取るれる時間と店舗までの道順を提示  
  (交通情報を含む複数の外部地図サービスとの統合が必要)
  - 要件２：店舗が宅配サービスを行っている場合、届先にサンドイッチを宅配
  - 要件３：モバイル端末でのアクセシビリティ
  - 要件４：全国向けにデイリープロモーション用メニューやスペシャルメニューを提供
  - 要件５：エリア限定でデイリープロモーション用メニューやスペシャルメニューを提供
  - 要件６：オンラインか直性、または配達時の支払いが可能
- 追加コンテキスト
  - 要件７：サンドイッチ点はフランチャイズで、オーナーはそれぞれ異なる
  - 要件８：親会社は近日中に海外展開を予定
  - 要件９：安い労働力を雇って利益を最大化するのがゴール

#### 5.3.1 明示的な特性

- ユーザ数：現在は数千だが、将来的には数百万
  - スケーラビリティ：必須
  - 弾力性：要検討
    - ユーザ数は時間によらずある程度一定か？
    - イベントや時間帯により激しく変動するのか？
- 要件１：注文から受け取り時間、道順を提示
  - 信頼性：要検討
    - 外部地図サービスが落ちた場合にどうなるのか
      - シリコンサンドイッチのサイトも落ちるからサポートするべきか
      - 多少効率が落ちるだけなのでサポートしないでもよいか
- 要件２：宅配サービス対応
  - 特になし
- 要件３：モバイル対応
  - 設計：必須
    - 各端末に対応した複数のネイティブアプリの構築は困難
    - ポータブルなWEBアプリケーションが良い
  - WEBアプリケーション向けパフォーマンス：要検討
- 要件４、５：カスタマイズ
  - カスタマイズ性：要検討
    - アーキで対応するか、設計で対応するか要検討
      - アーキ：マイクロカーネル
      - 設計：テンプレートメソッドなどのデザインパターン
- 要件６：支払い
  - セキュリティ：必須
    - オンライン決済時の暗号化
- 要件７：複数オーナー
  - コスト制約：要検討
    - オーナーの参加障壁を低くする必要があるか
- 要件８：海外展開
  - 国際化対応：設計で対応可能
- 要件９：安い労働力
  - ユーザビリティ：必須
    - スキルや訓練を必要としない安いアルバイトでも対応可能にする

#### 5.3.2 暗黙的な特性

- 可用性：ユーザが確実にアクセス・利用可能
- 信頼性：ユーザ操作中に稼働し続ける
  - 何度もログインし直しなどにならない
- セキュリティ：
